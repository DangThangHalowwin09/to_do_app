...
// Đường dẫn tới thư mục chứa file XML
const folderPath = "/Users/macbook/Documents/xml_database";

app.get("/latest-xml", async (req, res) => {
  try {
    if (!fs.existsSync(folderPath)) {
      return res.status(404).json({ error: `Thư mục ${folderPath} không tồn tại` });
    }

    const files = fs.readdirSync(folderPath).filter(f => f.endsWith(".xml"));
    if (files.length === 0) {
      return res.status(404).json({ error: "Không có file XML nào trong thư mục" });
    }
    ...
    res.status(500).json({ error: `Lỗi parse XML: ${err.message}` });
  }
});
